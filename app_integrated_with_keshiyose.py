...
def build_straight_results(target_df):
...
if st.button("統合処理を実行", type="primary", use_container_width=True):
    try:
        ...
        straight_detail, straight_composite = build_straight_results(target_df)
        ...
        st.success(f"直線詳細 {len(straight_detail)}頭 / 直線推奨 {len(straight_composite)}レース / 鉄板 {len(teppan_result)}頭 / 消寄 {len(keshi_result)}頭 / 表示 {len(cards)}レース")
