...
APP_VERSION = "integrated_sns_3venues_v3_fontfix_2026-05-15"
...
FONT_CACHE_DIR = Path("/tmp/kettou_fonts")
FONT_CACHE_DIR.mkdir(parents=True, exist_ok=True)

JP_FONT_REGULAR_CANDIDATES = [
    APP_DIR / "NotoSansCJKjp-Regular.otf",
...
JP_FONT_BOLD_CANDIDATES = [
    APP_DIR / "NotoSansCJKjp-Bold.otf",
    APP_DIR / "NotoSansJP-Bold.ttf",
...
# PIL画像生成
# =========================================================
def resolve_font_path(bold=False):
    candidates = JP_FONT_BOLD_CANDIDATES if bold else JP_FONT_REGULAR_CANDIDATES
    for p in candidates:
        try:
            if Path(p).exists():
                return str(p)
...
def get_font(size, bold=False):
    ...
    if path:
        try:
            ...
            pass
    # 最終フォールバック。日本語は豆腐になる可能性ありだが、アプリは止めない。
