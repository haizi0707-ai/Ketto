...
HEADERLESS_COLS = [
    ...
    '前走馬場状態','前走芝ダ','前走距離','前走斤量','休み明け〜戦目','所属','調教師','騎手','前走騎手',
...
def calc_prev_straight_score(row):
    ...
    corner_score = {'2〜3番手':12,'4〜6番手':14,'7〜10番手':8,'11番手以下':5,'1番手':4}.get(cat4, 0)
    style = str(row.get('前走脚質',''))
    if '先' in style or '好位' in style: corner_score += 4
    elif '差' in style: corner_score += 3
    elif '追' in style: corner_score += 1
    c3 = to_int(row.get('前走通過順3')); c4 = to_int(row.get('前走通過順4'))
    if c3 is not None and c4 is not None:
        if c4 < c3: corner_score += 4
        elif abs(c4 - c3) <= 1: corner_score += 3
    score += min(corner_score, 20)
    ...
    prev_surface = str(row.get('前走芝ダ','')); cur_surface = str(row.get('芝ダ',''))
    cond = 0
    ...
    if margin is not None and margin >= 1.0: penalty -= 6
...
def build_straight_results(target_df, min_score):
    ...
    for _, g in df.groupby(['日付','競馬場','R_num'], sort=False):
        ...
        rows.append({'日付':main['日付'],'競馬場':main['競馬場'],'R':int(main['R']),'レース名':main['レース名'],'馬番':int(main['馬番']) if pd.notna(main['馬番']) else '', '馬名':main['馬名'], '信頼度':int(main['信頼度']), '印':'◎','対抗':taikou,'対抗馬名':opp.iloc[0]['馬名'] if not opp.empty else '', '単穴':tanketsu,'単穴馬名':ana.iloc[0]['馬名'] if not ana.empty else '', '連下':ren_nums[0] if len(ren_nums)>0 else '', '連下馬名':ren.iloc[0]['馬名'] if len(ren_nums)>0 else '', '他1':ren_nums[1] if len(ren_nums)>1 else '', '他2':ren_nums[2] if len(ren_nums)>2 else '', '相手表示':''})
...
def render_html(straight_composite, image_date):
    ...
    for _, r in straight_composite.iterrows():
        ...
        for mark, ncol, nmcol in [('○','対抗','対抗馬名'),('▲','単穴','単穴馬名'),('△','連下','連下馬名')]:
            if norm_text(r.get(ncol,'')): lines.append((mark, r.get(ncol,''), r.get(nmcol,'')))
...
if st.button('統合処理を実行', type='primary', use_container_width=True):
    try:
        if mode == 'ファイル読み込み':
            if uploaded is None:
                st.warning('CSVファイルを選択してください。'); st.stop()
            target_df = read_csv_smart(uploaded)
        else:
            if not pasted.strip():
                st.warning('CSV本文を貼り付けてください。'); st.stop()
