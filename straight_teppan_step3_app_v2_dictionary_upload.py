...
APP_VERSION = 'straight_teppan_step3_v2_dictionary_upload_2026-05-15'
...
def detect_teppan_files():
    """
    app.pyと同じ場所にある鉄板辞書CSVを自動検出する。
    ファイル名が少し省略・変更されていても拾えるようにする。
    """
    files = sorted(Path(".").glob("*.csv"), key=lambda p: p.name)
    ...
    for f in files:
        name = f.name
        if ("TOP5" in name or "TOP５" in name) and ("最終ロジック" in name or "ロジック辞書" in name or "修正版" in name):
            logic_path = f
        if "コース別" in name and ("採用" in name or "保留" in name or "除外" in name or "判定" in name):
            course_path = f

    # 完全一致優先
    exact_logic = Path("鉄板血統_TOP5並列_修正版_最終ロジック辞書.csv")
    exact_course = Path("鉄板血統_コース別_採用保留除外判定.csv")
    if exact_logic.exists():
        logic_path = exact_logic
    if exact_course.exists():
        course_path = exact_course

    return logic_path, course_path

def read_any_csv(obj):
    for enc in ['utf-8-sig', 'cp932', 'shift_jis', 'utf-8']:
        try:
            ...
            pass
    raise ValueError(f'CSVを読めません: {getattr(obj, "name", obj)}')
...
def build_teppan_results(target_df, use_statuses, logic_file=None, course_file=None):
    # 1) 画面でアップロードされた辞書CSVを優先
    if logic_file is not None and course_file is not None:
        logic_df = read_any_csv(logic_file)
        course_df = read_any_csv(course_file)
        logic_path = getattr(logic_file, 'name', 'uploaded_logic.csv')
        course_path = getattr(course_file, 'name', 'uploaded_course.csv')
    else:
        # 2) app.pyと同じ場所から自動検出
...
st.markdown('### 鉄板辞書CSV')
st.caption('app.pyと同じ場所に置けない場合は、ここで2つのCSVを選択してください。未選択なら同じフォルダから自動検出します。')
logic_upload = st.file_uploader('鉄板ロジック辞書CSV', type=['csv'], key='logic_upload')
course_upload = st.file_uploader('鉄板コース判定CSV', type=['csv'], key='course_upload')
...
if uploaded is not None:
    try:
        ...
        teppan_result, logic_path, course_path = build_teppan_results(raw_df, use_statuses, logic_upload, course_upload)
        ...
        if logic_path is None or course_path is None:
            ...
            st.info(f'鉄板辞書: {getattr(logic_path, "name", logic_path)} / コース判定: {getattr(course_path, "name", course_path)}')
